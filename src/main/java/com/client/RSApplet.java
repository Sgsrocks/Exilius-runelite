...
public class RSApplet extends Applet implements Runnable, MouseListener, MouseMotionListener, MouseWheelListener,
	...
	void drawLoadingText(int percentage, String s, int downloadSpeed, int secondsRemaining) {
		...
		graphics.fillRect(0, 0, Client.currentGameWidth, Client.currentGameHeight);
		shouldClearScreen = false;
		//}
		Color color = new Color(140, 17, 17);
		int j = Client.currentGameHeight / 2 - 18;
		graphics.setColor(color);
		graphics.drawRect(Client.currentGameWidth / 2 - 152, j, 304, 34);
		graphics.fillRect(Client.currentGameWidth / 2 - 150, j + 2, percentage * 3, 30);
		graphics.setColor(Color.black);
		graphics.fillRect((Client.currentGameWidth / 2 - 150) + percentage * 3, j + 2, 300 - percentage * 3, 30);
		graphics.setFont(font);
		graphics.setColor(Color.white);
		graphics.drawString(s, (Client.currentGameWidth - fontmetrics.stringWidth(s)) / 2,
				j + 22);
		...
		if(downloadSpeed != -1 && secondsRemaining != -1) {
			...
			graphics.drawString(text, (Client.currentGameWidth - fontmetrics.stringWidth(text)) / 2, j + 70);
		}
	}
