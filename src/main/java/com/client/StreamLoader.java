...
public class StreamLoader {
	...
	public byte[] getArchiveData(String s) {
		if (Configuration.loadExternalCacheArchives && new File(Signlink.findcachedir() + Configuration.EXTERNAL_CACHE_ARCHIVE + s).exists()) {
			try {
				//System.out.println("Loaded external file: " + s);
				return getBytesFromFile(new File(Signlink.findcachedir() + Configuration.EXTERNAL_CACHE_ARCHIVE + s));
			} catch (IOException e) {
				e.printStackTrace();
