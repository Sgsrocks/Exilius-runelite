...
public class SettingsManager {
	...
	public static void saveSettings(Client client) throws IOException {
		...
		ObjectOutputStream output = new ObjectOutputStream(new FileOutputStream(new File(Signlink.findcachedir() + "settings.ser")));
		try {
	...
	public static void loadSettings() {
        try {
        	File discover = new File(Signlink.findcachedir() + "settings.ser");
        	if (!discover.exists()) {
        		Client.setUserSettings(Settings.getDefault());
        		return;
        	}

        	try (ObjectInputStream input = new ObjectInputStream(new FileInputStream(Signlink.findcachedir() + "settings.ser"))) {
